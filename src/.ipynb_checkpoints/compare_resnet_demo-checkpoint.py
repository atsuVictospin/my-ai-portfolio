...
train_data = datasets.ImageFolder("..\datasets\mvtec_ad\processed\metal_nut\metal_nut\train", transform=transform)
test_data = datasets.ImageFolder("..\datasets\mvtec_ad\processed\metal_nut\metal_nut\val", transform=transform)
...
def create_resnet(num_classes):
    model = models.rsneet18(pretrained=True)    # 事前学習済みモデル
    model.fc = nc.Linear(model.fc.in_features, num_classes) # 出力層の差し替え
    return model
